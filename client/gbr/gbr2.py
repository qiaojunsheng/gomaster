...
class GbrDebugDlg(GrDialog):
    ...
    def save_debug_info(self):
        ...
        messagebox.showinfo("调试图像", "{} 个文件已保存到 {}".format(n, dn))
...
class GbrStonesDlg(GrDialog):
    ...
    def init_buttons(self, buttonFrame):
        ...
        tk.Checkbutton(f_top, text = "全选", variable = self.allVar,
            command = self.select_all_callback).pack(side = tk.LEFT, padx = 5, pady = 5)

        NButton(f_bottom, text = "保存SGF",
...
class GbrGUI2(tk.Tk):
    ...
    def __init_toolbar(self):
        ...
        ImgButton(self.toolbarPanel,
            tag = "open", tooltip = "打开图像 (Ctrl-O)", key='<Control-Key-o>',
            command = self.open_image_callback).pack(side = tk.LEFT, padx = 2, pady = 2)

        ImgButton(self.toolbarPanel,
            tag = "edge", tooltip = "变换图像 (Ctrl-E)", key='<Control-Key-e>', disabled = True,
            command = self.transform_callback).pack(side = tk.LEFT, padx = 2, pady = 2)

        ImgButton(self.toolbarPanel,
            tag = "area", tooltip = "定义棋盘区域 (Ctrl-A)", key='<Control-Key-a>', disabled = True,
            command = self.set_area_callback).pack(side = tk.LEFT, padx = 2, pady = 2)

        ImgButton(self.toolbarPanel,
            tag = "grid", tooltip = "设置棋盘网格 (Ctrl-G)", key='<Control-Key-g>', disabled = True,
            command = self.set_grid_callback).pack(side = tk.LEFT, padx = 2, pady = 2)

        ImgButton(self.toolbarPanel,
            tag = "detect", tooltip = "检测棋子 (Ctrl-D)", key='<Control-Key-d>', disabled = True,
            command = self.detect_callback).pack(side = tk.LEFT, padx = 2, pady = 2)

        ImgButton(self.toolbarPanel,
            tag = "stones", tooltip = "棋子列表 (Ctrl-L)", key='<Control-Key-l>', disabled = True,
            dlg_class = GbrStonesDlg).pack(side = tk.LEFT, padx = 2, pady = 2)

        ImgButton(self.toolbarPanel,
            tag = "params", tooltip = "检测参数 (Ctrl-P)", key='<Control-Key-p>', disabled = True,
            dlg_class = GbrOptionsDlg).pack(side = tk.LEFT, padx = 2, pady = 2)

        ImgButton(self.toolbarPanel,
            tag = "save", tooltip = "保存为SGF (Ctrl-S)", key='<Control-Key-s>',disabled = True,
            command = self.save_sgf_callback).pack(side = tk.LEFT, padx = 2, pady = 2)

        ImgButton(self.toolbarPanel,
            tag = "reset", tooltip = "重置图像", disabled = True,
    ...
    def open_image_callback(self, event: tk.Event):
        """Open button click"""
        fn = filedialog.askopenfilename(title = "选择文件",
...
##                self.statusBar.set("")
    ...
    def mouse_click_callback(self, event):
        ...
        if self.board.is_gen_board:
            self.statusBar.set("未加载棋盘图像")
        elif len(self.board.stones.unforced_stones()) == 0:
            self.statusBar.set("需要处理棋盘图像，请点击检测按钮")
        else:
            x, y = self.imagePanel.frame2image((event.x, event.y))
    ...
    def load_image(self, filename):
        ...
        if self.log.errors > 0:
            self.statusBar.set("文件加载时出错，点击此处查看日志")
        else:
            self.statusBar.set_file("文件已加载:", self.board.image_file)
    ...
    def detect_stones(self, highlight = False):
        ...
        # Update status
        if self.log.errors > 0:
            self.statusBar.set("处理时出错，点击此处查看日志")
        else:
            self.statusBar.set("检测到 {b} 个黑子, {w} 个白子，棋盘大小 {s}x{s}，点击此处查看日志".format(
    ...
    def save_sgf(self, fn):
        ...
        if not self.board.is_gen_board:
            ...
            self.statusBar.set_file("棋盘已保存到 ", str(fn))

    def show_stone(self, stone):
        """Highlight one stone"""
        stone_color = "黑子" if stone[GR_BW] == STONE_BLACK else "白子"
        self.statusBar.set("{} {}".format(stone_color, format_stone_pos(stone)))
